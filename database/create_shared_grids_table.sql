...
-- Optional: Add a policy to automatically delete old shares (e.g., after 30 days)
-- You can uncomment this if you want to automatically clean up old shares
/*
CREATE OR REPLACE FUNCTION delete_old_shares() RETURNS void AS $$
BEGIN
    DELETE FROM shared_grids WHERE created_at < NOW() - INTERVAL '30 days';
END;
$$ LANGUAGE plpgsql;

-- Create a scheduled job to run the cleanup function daily
-- Note: This requires the pg_cron extension to be enabled
SELECT cron.schedule('delete-old-shares', '0 2 * * *', 'SELECT delete_old_shares();');
*/
