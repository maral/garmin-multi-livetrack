...
import {
    groupIdentifiersByProvider,
    parseTrackingUrlsAsync,
} from "./urlParser";
...
export async function expandUrls(
    request: ExpandUrlsRequest,
): Promise<ExpandUrlsResponse> {
    const results = await parseTrackingUrlsAsync(request.urls);
    return { results };
}
...
export async function fetchTrackingData(
    ...
    const providerPromises = Object.entries(groups).map(
        async ([providerType, identifiers]) => {
            try {
                ...
                console.error(
                    `Error fetching data from ${providerType}:`,
                    error,
                );
                return [];
            }
...
export async function fetchTrackingUpdates(
    ...
    const begin = new Date(
        request.begin ? request.begin : Date.now() - 30 * 1000,
    );
    ...
    const providerPromises = Object.entries(groups).map(
        async ([providerType, identifiers]) => {
            try {
                const provider = getProvider(providerType as TrackingType);
                const data = identifiers.map((id) => id.data);
                return await provider.fetchTrackingUpdates(data, begin);
            } catch (error) {
                console.error(
                    `Error fetching updates from ${providerType}:`,
                    error,
                );
                return [];
            }
