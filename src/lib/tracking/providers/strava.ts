 ...
 */
async function fetchSingleStravaAthlete(
    beaconId: string,
    begin?: string,
 ...
 */
async function fetchSingleStravaAthleteUpdates(
    beaconId: string,
    begin: Date,
 ...
 */
function convertStravaToUnified(
    stravaData: StravaTrackingData,
): UnifiedTrackingData {
    const coordinates: UnifiedCoordinate[] = stravaData.coordinates.map(
        (coord) => ({
...
export const stravaProvider: TrackingProvider = {
    ...
    async fetchTrackingData(
        identifiers: StravaBeaconData[],
        begin?: string,
    ): Promise<UnifiedTrackingData[]> {
        const results = await Promise.allSettled(
            identifiers.map(({ beaconId }) =>
                fetchSingleStravaAthlete(beaconId, begin)
            ),
        );
        ...
        return results
            .filter((
                result,
            ): result is PromiseFulfilledResult<StravaTrackingData> =>
    ...
    async fetchTrackingUpdates(
        ...
        const results = await Promise.allSettled(
            identifiers.map(({ beaconId }) =>
                fetchSingleStravaAthleteUpdates(beaconId, begin)
            ),
        );
        ...
        return results
            .filter((
                result,
            ): result is PromiseFulfilledResult<StravaCoordinate[]> =>
                result.status === "fulfilled"
            )
            .map((result, index) => ({
                id: `https://www.strava.com/beacon/${
                    identifiers[index].beaconId
                }`,
