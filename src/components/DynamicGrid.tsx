...
export default function DynamicGrid() {
  ...
  const calculateOptimalDimensions = (cellCount: number, currentRows: number, currentCols: number) => {
    ...
    for (let testRows = Math.max(currentRows, 1); testRows <= maxDimension; testRows++) {
    ...
    for (let testCols = Math.max(currentCols, 1); testCols <= maxDimension; testCols++) {
  ...
  const handleBulkImport = (urls: string[]) => {
    ...
    const { rows: newRows, cols: newCols } = calculateOptimalDimensions(urls.length, rows, cols);
    ...
    setCellData(prevData => {
      ...
      let urlIndex = 0;
      for (let rowIndex = 0; rowIndex < newRows && urlIndex < urls.length; rowIndex++) {
        for (let colIndex = 0; colIndex < newCols && urlIndex < urls.length; colIndex++) {
          const key = `${rowIndex}-${colIndex}`;
          newData[key] = {
            url: urls[urlIndex],
            isEditing: false
